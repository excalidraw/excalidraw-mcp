...
import { exportJsonToExcalidrawUrl } from "./export-to-excalidraw.js";
...
2) decide whether you want to make new diagram from scratch OR - use this one as starting checkpoint:
  ...
  registerAppTool(server,
    ...
    async ({ json }): Promise<CallToolResult> => {
      ...
      try {
        const url = await exportJsonToExcalidrawUrl(json);
  ...
  // CSP: allow Excalidraw fonts and export API uploads from the widget sandbox
  const cspMeta = {
    ui: {
      csp: {
